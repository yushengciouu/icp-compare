...
### 📊 語意比對與風險判定矩陣 (Decision Matrix)

| 查詢名稱 / 別名比對 | 物理地址比對 | 最終判定等級 | 風險判定依據 (risk_factor) |
| :--- | :--- | :--- | :--- |
| **主名稱命中** | **地址命中** | 🔴 **High** | `兩者均相同` |
| **別名 (Alias) 命中** | **地址命中** | 🔴 **High** | `兩者均相同` |
| **主名稱不同** | **出貨地址完全相同** | 🔴 **High** | `地址相同（名稱不同）` |
| **主名稱命中** | **地址不同** | 🟠 **Medium** | `名稱相同（地址不同）` |
| **別名 (Alias) 命中** | **地址不同** | 🟠 **Medium** | `別名/轉投資命中` |
| **名稱 / 別名皆無關** | **地址無關** | 🟢 **False Positive** | *(空字串)* |
